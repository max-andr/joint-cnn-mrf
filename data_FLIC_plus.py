...
"""
This script generate the x_train, x_test, y_train, y_test for the further process from the FLIC_plus dataset.
...
There're 17380 for training and 20928-17380 for testing
"""
...
data_FLIC = loadmat('data_FLIC_plus.mat')
...
train_index = loadmat('tr_plus_indices.mat')
train_index = train_index['tr_plus_indices']
train_index = list(np.reshape(train_index, [len(train_index)]))
test_index = loadmat('ts_plus_indices.mat')
test_index = test_index['ts_plus_indices']
test_index = list(np.reshape(test_index, [len(test_index)]))
...
for i in train_index:
    img = misc.imread('./images_FLIC_plus/' + data_FLIC[i][3][0])
...
np.save('x_train_flic_plus', x_train)
np.save('x_test_flic_plus', x_test)
...
np.save('y_train_flic_plus', x_train_hmap)
...
np.save('y_test_flic_plus', x_test_hmap)
